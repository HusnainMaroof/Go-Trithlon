...
export const getAllAthlete = catchErrors(async (): Promise<ActionResponse> => {
  // 1. Get the index of userIds
  const userIds = await setRedis.lrange("athletes:index", 0, 99); // first 100

  if (userIds && userIds.length > 0) {
    // 2. Fetch all individual entries in one round-trip via pipeline
  ...
  // 3. Cold start — nothing in cache, hit DB
  ...
  // 4. Backfill cache — write index and individual entries
  if (formatted.length > 0) {
    const pipeline = setRedis.pipeline();

    for (const athlete of formatted) {
    ...
    // Index expires in 1 hour — forces periodic re-validation
