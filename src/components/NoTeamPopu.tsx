...
export const RecruitSlotModal = ({
  ...
  const isMyTeam = FromWhere === "MyTeamDashboard";
  const userHasSkill = userDisciplines.includes(slot);
      ...
      >
        ...
        <div className="flex flex-col mt-2 relative z-10">
          ...
          <div className="flex flex-col gap-3 w-full">
            {/* ── MyTeamDashboard: full claim flow ── */}
            {isMyTeam && (
              <>
                {userHasSkill ? (
            ...
            {/* ── MainDashboard: skill info only, no claim ── */}
            {!isMyTeam && (
              <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex flex-col gap-3">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-11 h-11 rounded-full ${config.bg} ${config.border} border flex items-center justify-center shrink-0`}
                  >
                    {React.createElement(config.icon, {
                  ...
                  <div>
                    <h4 className="font-bold text-zinc-300 text-sm">
                      {userHasSkill
                        ? `You're a ${config.label}`
                        : "Skill Not on Your Profile"}
                    </h4>
                    <p className="text-xs text-zinc-500 mt-0.5">
                      {userHasSkill
                        ? "You can claim this spot from your team page."
                        : `You don't have ${config.label} listed.`}
                    </p>
                  </div>
                </div>

                {/* Always show current skills in MainDashboard */}
                <div className="border-t border-zinc-800 pt-3 mt-1">
                  <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-2">
                    Your Current Skills
                  </p>
            ...
            {/* ── Search for athlete — always shown ── */}
